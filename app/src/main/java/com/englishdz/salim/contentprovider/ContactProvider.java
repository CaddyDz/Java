...
// Contact Provider allow applications to share data using an API
// The interface provides access using delete, insert, select and update methods

public class ContactProvider extends ContentProvider {

    // The Java namespace for the Content Provider
    static final String PROVIDER_NAME = "com.englishdz.salim.contentprovider.ContactProvider";
    ...
    static {
        ...
        URI_MATCHER.addURI(PROVIDER_NAME, "cpcontacts", uriCode);
    }
    ...
    // Uri : Links to the table in the provider (The Form part of a query)
    ...
    public Uri insert(@NonNull Uri uri, @Nullable ContentValues values) {
        ...
        // Gets the row id after inserting a map with the keys representing the column names and their values.
        // The second attribute is used when you try to insert an empty row
        long rowID = sqlDB.insert(TABLE_NAME, null, values);

        // Verify a row has been added
        if (rowID > 0) {

            // Append teh given id to the path and return a Builder used to manipulate URI references
